...
`import_tool` 只负责整包收编，**不生成适配层**。被接入的外部项目必须自带根级 `cli.py`
并满足以下接口约定，否则会接入成功但运行期报「外部工具入口不存在」或参数不被识别：

1. **入口文件**：项目根目录必须存在 `cli.py`（复制后为 `external/<name>/cli.py`），网关固定以 `python cli.py` 启动；
2. **参数格式**：`cli.py` 必须支持重复的 `--param key=value` 形式（网关把调用参数逐项拼成 `--param k=v` 追加在命令末尾，不传 `--tool`）；
3. **返回约定**：成功以退出码 `0` 结束，输出写入 stdout / stderr（各截断 2000 字符）。

不符合该约定的项目（例如入口为 `scripts/xxx.py` 的子命令式 CLI）无法直接执行，需先将其包装为符合上述约定的 `cli.py` 后再接入。

> **适用边界**：本网关面向「一次性、有界、可文本化」的批处理类工具——文件生成、数据处理、
> 批处理脚本等都在边界内。交互式、流式、常驻进程或依赖实时反馈的工具（典型如桌面控制、
> GUI 自动化）超出边界，需要另一套执行机制，而非通过调整 `--param` 约定来迁就。
